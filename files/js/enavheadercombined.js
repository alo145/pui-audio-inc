...
var __headerLayout = {
    ...
    setCart: function (title, body, data) {
        ...
        for (i = 0; i < effective; i++) {
            ...
            if (shippedByName === "") {
                shippedByName = __headerData.shippedByDefault;
            }
...
if (document.querySelector('.flymenu__clc-lang .flymenu__radio-container input')) {
    __helpers.onEvent(document.querySelectorAll('.flymenu__clc-lang .flymenu__radio-container input'), 'change', function (e) { var input = this; __langCurr.invokeChange(input.dataset.lang, __headerLayout.effectiveCur); });
}
...
if (document.querySelectorAll('.lower-currencies a')) {
    __helpers.onEvent(document.querySelectorAll('.flymenu__clc-curr a'), 'click', function (e) { displayCurPrompt(); });
}

//put a copy of the graybar messaging inside the header to float alongside the flyout navs
...
if (graybar) {
    var graybarCopy = document.createElement("div");
    graybarCopy.setAttribute("id","banner__graybar--inNav");
    graybarCopy.innerHTML = graybar.innerHTML
    var putHere = document.querySelector(".flymenu__nav-bar")
    insertAfter(graybarCopy, putHere)

        var marquees = document.querySelectorAll(".graybar--marquee");
        if (marquees.length > 2) {
            var slideIndex = 0;
            var inNavSlides = document.querySelectorAll("#banner__graybar--inNav .graybar--marquee");
            var grayBarSlides = document.querySelectorAll("#banner__graybar .graybar--marquee");

            function runMarquee() {
                for (o = 0; o < grayBarSlides.length; o++) {
